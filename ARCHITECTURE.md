...
| `control_inputs.py` | `ControlInputsPanel` — [C]_М, h_c (0,8–3,3 м), П_O₂ авто/ручн.; i, τ; V; η·Z; «Симулировать» / «Применить»; пресеты v3 |
| `melt_dynamics.py` | `MeltDynamicsEngine` — пошаговая динамика Δt=30 с, Ф-Д1…Ф-Д8, `preview_free_params` для баланса |
| `dynamics_charts.py` | `DynamicsChartsWidget` — 4 графика pyqtgraph |
| `dynamics_indicators.py` | `DynamicsIndicatorsPanel` — η·Z, Φ, невязка [C] |
...
## Трёхслойная схема (баланс / динамика / связка) — v3

| Слой | Модуль | Когда |
|------|--------|-------|
| Баланс Шаповалова | `OperForm` этапы 1–8 | «Применить и пересчитать», кнопка сценария |
| Динамика | `melt_dynamics.MeltDynamicsEngine` | «Симулировать плавку», шаг 30 с |
| Связка | `_resolve_dynamic_free_params()` | `η_CO`, `Z`, `П_O₂` в `blastCalcClicked`, `HeatBalanceCalcClicked` |

Свободные параметры считаются от `h_c` (0,8–3,3 м) и `i_уд` (Ф-Д2…Ф-Д6), а не от устаревших формул с `h_c_max=4,5 м`.

Центральная колонка: только «Последовательность расчётов» (8 этапов + «Запустить все этапы»). Вкладка **«Симуляция»** (после «Схема»): `ControlInputsPanel` → read-only η/Z/П_O₂/K_П/G_В → графики → индикаторы (`QSplitter` в прокручиваемой области).

Зависимость графиков: `pip install -r requirements-dynamics.txt`.
...
**Вход:** массы и составы чугуна и лома из полей формы; целевой `[C]_М` — с панели (`_get_target_carbon()`).

**Формулы:**
- При изменении `[C]_М` на панели: `G_лом` пересчитывается от базы сценария (~2% на 0,05% Δ[C], разд. 3); полная формула (3) не используется при близких C_лом и [C]_М
...
**Вход:** химический состав шихты, целевой состав стали (`_get_target_carbon()` и др.).
...
- `O₂_дожигание CO` — формула (20): `× η_CO/100 × 16/28`, η_CO от h_c (Ф-A, 5–15 %)
- `O₂_флюсы` — вычитается O₂ из FeO/Fe₂O₃ флюсов
- `O₂_итого = O₂_общ + O₂_железо + O₂_дожигание − O₂_флюсы`
- `G_дутья (кг)` — формула (25): `O₂_итого × (1 + П_O2/100) × 100 / 99.5`, П_O2 с панели (5–10 %)
- `V_дутья (м³) = G_дутья × 22.4/32` — **только из баланса O₂**, не с крутилки
- Избыток дутья: 8%
- Расход `i` и время `τ` с панели — **информационные** (3D, отображение), связь через Ф-2 после расчёта V

**Выход:** `TotalOxygenDemandBlast`, `TotalConsumptionOfBlastKg`, `TotalConsumptionOfBlastM3`, `ExcessBlast`; read-only V на панели.

**3D:** `blastFlow` (информационный i).
...
- Дожигание CO → CO₂: доля `η_CO/100` (формула 20), η_CO от h_c
...
| Дожигание CO | формула (36): `Q_CO = 101 × g_CO × η_CO × Z`, η_CO и Z от h_c (Ф-A, Ф-B) |
